# Research Docs

This folder contains the reusable research system behind `VR-apps-lab`.

The goal is to make the repository useful not just as a codebase, but as a
curated knowledge base for VR utilities, overlays, runtime tools, tracker
bridges, device managers, and experimental XR workflows.

## Subfolders

- `methods/`  
  Extracted implementation methods and product approaches for building VR
  utilities.

- `discovery/`  
  Discovery sources, intake flow, and watchlists for finding new repositories.

- `landscape/`  
  Cross-project landscape research, waves, overlap families, and synthesis.

- `catalog/`  
  Canonical registry of tracked repositories and study coverage.

- `program/`  
  The research operating model: restructuring plan, backlog, and study method.

- `reuse/`  
  Focused reuse-plan documents for specific donor repositories.

- `templates/`  
  Reusable templates for future project studies.

## Recommended reading order

1. `../foundation/current-operating-context.md`
2. `program/new-session-quickstart.md`
3. `methods/vr-utility-methods-catalog.md`
4. `landscape/project-families.md`
5. `catalog/project-registry.md`
6. `program/repository-restructuring-plan.md`
7. `program/restructuring-backlog.md`
8. `program/github-research-wave-8-plan.md`
9. `program/github-research-wave-9-plan.md`
10. `program/github-research-wave-10-plan.md`
11. `program/github-research-wave-11-plan.md`
12. `program/github-research-wave-12-plan.md`
13. `landscape/vr-projects-wave-12-synthetic-devices-input-emulation-and-diy-driver-paths.md`
14. `program/github-research-wave-13-plan.md`
15. `landscape/vr-projects-wave-13-vision-tracking-hand-bridges-and-headsetless-camera-runtimes.md`
16. `program/github-research-wave-14-plan.md`
17. `landscape/vr-projects-wave-14-tracker-ingress-osc-egress-and-role-binding-utilities.md`
18. `program/github-research-wave-15-plan.md`
19. `landscape/vr-projects-wave-15-overlay-hosts-and-scaffolds.md`
20. `reuse/h-view-overlay-host-reuse-plan.md`
21. `program/foundational-waves-1-7-retro-normalization-plan.md`
22. `landscape/vr-projects-foundational-waves-1-7-retro-normalization.md`
23. `discovery/local-source-cache-workflow.md`
24. `discovery/intake-pipeline.md`
25. `landscape/not-yet-studied-deeply.md`
